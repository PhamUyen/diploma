...
public class ResizeSurfaceView extends SurfaceView {
     ...
     * @param videoHeight
     */
    public void adjustSize(int surfaceViewWidth, int surfaceViewHeight, int videoWidth, int
            videoHeight) {
        if (videoWidth > 0 && videoHeight > 0) {
            ...
            int margin = (int) (getContext().getResources().getDisplayMetrics().density *
                    MARGIN_DP);
            float videoRatio = 0;
            ...
            if (windowWidth < windowHeight) {// portrait
                if (videoWidth > videoHeight) {
                    if (surfaceViewWidth / videoRatio > surfaceViewHeight) {
                        lp.height = surfaceViewHeight +margin;
                        lp.width = (int) ((surfaceViewHeight + margin) * videoRatio);
                    } else {
